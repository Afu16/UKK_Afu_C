<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl">Kartu Siswa</h2></x-slot>
    ...
    <div class="py-8 max-w-sm mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow p-6 text-center" id="kartu">
            ...
            <div class="text-sm text-gray-500 mb-1">{{ $user->rombel->nama_rombel ?? '-' }} — {{ $user->rayon->nama_rayon ?? '-' }}</div>
            <div class="text-xs text-gray-400 mb-4">No ID: {{ $user->no_id ?? '-' }}</div>
            ...
            <div class="text-xs text-gray-400">{{ $user->no_id }}</div>
        </div>
    ...
    <script>
        document.title = "Kartu - {{ $user->name }}";
        JsBarcode("#barcode", "{{ $user->no_id ?? 'NO-ID' }}", {
            format: "CODE128",
            width: 2,
            height: 60,
            displayValue: true,
